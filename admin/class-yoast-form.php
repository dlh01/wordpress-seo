...
class Yoast_Form {
	...
	public function textinput( $var, $label, $attr = array() ) {
		...
		( isset( $attr['disabled'] ) && $attr['disabled'] ) ? ' disabled' : '',
		' class="' . esc_attr( 'textinput ' . $attr['class'] ) . '"',
		// phpcs:ignore WordPress.Security.EscapeOutput -- Ternary operators are not handled correctly by the Sniff. See https://github.com/WordPress/WordPress-Coding-Standards/issues/677 and https://github.com/WordPress/WordPress-Coding-Standards/issues/1219.
		( $attr['placeholder'] !== '' ) ? ' placeholder="' . esc_attr( $attr['placeholder'] ) . '"' : '',
		' id="', esc_attr( $var ), '"',
